...
from corr_filter_tracker import CorrFilterTracker, CFParams
...
vot13seq = os.listdir('../data/vot14/')
sequences = vot13seq
...
if __name__ == "__main__":
    for sequence in sequences:
        if sequence in ["list.txt", "ball"]:
            ...
            parameters = CFParams()
            tracker = CorrFilterTracker(parameters)
            trackerType = "CF"
            ...
            avg_failure += n_failures / len(sequences)
            total_failures += n_failures
    print(f"avg failure: {avg_failure}")
